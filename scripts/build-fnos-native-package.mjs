...
function args() { const values={}; for(let i=2;i<process.argv.length;i+=2) values[process.argv[i]]=process.argv[i+1]; if(!["x86","arm"].includes(values["--platform"])||!values["--vendor"]||!values["--web"]||!values["--version"]||!values["--output"]) fail("usage: --platform x86|arm --vendor <dir> --web <standalone> --version <version> --output <fpk>"); assertCurrentFnOSVersion(values["--version"]); return values; }
