...
async def test_document_job_sends_parsed_markdown_to_engine(monkeypatch):
    ...
    await tasks.process_document(FakeSession(), job, engine_manager=engine, job_queue=None)
...
async def test_document_job_persists_successful_mineru_outcome(
    ...
    await tasks.process_document(FakeSession(), job, engine_manager=FakeEngineManager(), job_queue=None)
...
async def test_document_job_persists_mineru_markitdown_fallback(monkeypatch, caplog):
    ...
    await tasks.process_document(
        FakeSession(), job, engine_manager=FakeEngineManager(), job_queue=None
    )
